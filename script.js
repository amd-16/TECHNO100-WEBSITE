...
function renderResults(results, query){
  ...
  if(!results.length){ div.innerHTML=`<div class="no-result"><div class="nr-icon">📖</div><p>No results found for <strong>"${query}"</strong></p></div>`; return }
  ...
  results.forEach(item=>{
    ...
    if(currentMode==='english'){
      ...
      const matches = otherData.filter(other=>getEnglish(other).toLowerCase()===english.toLowerCase())
      if(matches.length){
        ...
        crossHtml = `<div class="cross-refs"><div class="cross-refs-title">Also in ${otherLabel}</div>${crossCards}</div>`
      }
    }
...
function posClass(pos){if(!pos)return'pos-other';const p=pos.toLowerCase();if(p.includes('noun'))return'pos-noun';if(p.includes('verb'))return'pos-verb';if(p.includes('adj'))return'pos-adjective';if(p.includes('adv'))return'pos-adverb';return'pos-other'}
...
function buildAlpha(){
  ...
  container.innerHTML='ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').map(l=>`<button class="alpha-btn ${letters.includes(l)?'':'disabled'}" data-alpha="${l}">${l}</button>`).join('')
  container.querySelectorAll('.alpha-btn:not(.disabled)').forEach(btn=>{
    btn.addEventListener('click',()=>{
      const letter = btn.dataset.alpha
      if(activeAlpha===letter){activeAlpha=null;btn.classList.remove('active')}
      else{container.querySelectorAll('.alpha-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');activeAlpha=letter}
      wordsPage=1
      applyWordsFilter()
    })
  })
...
function renderWordsTable(){
  ...
  const rows = slice.map(item=>{
    ...
    return `<tr><td class="td-word">${hlWord}</td><td class="td-english">${hlEnglish}</td><td class="td-tagalog">${tagalog}</td><td><span class="td-pos ${posClass(pos)}">${pos||'—'}</span></td></tr>`
  }).join('')
  document.getElementById('wordsBody').innerHTML = rows || `<tr><td colspan="4" style="text-align:center;padding:32px;color:var(--muted)">No words found.</td></tr>`
}
...
function renderPagination(){
  ...
  const pages=[]
  for(let i=1;i<=total;i++){if(i===1||i===total||(i>=wordsPage-2&&i<=wordsPage+2))pages.push(i);else if(pages[pages.length-1]!=='…')pages.push('…')}
  pages.forEach(p=>{if(p==='…')html+=`<span class="pg-ellipsis">…</span>`;else html+=`<button class="pg-btn ${p===wordsPage?'active':''}" onclick="goPage(${p})">${p}</button>`})
...
function goPage(p){const total=Math.ceil(wordsFiltered.length/PAGE_SIZE);if(p<1||p>total)return;wordsPage=p;renderWordsTable();renderPagination();document.querySelector('.words-table-wrap').scrollIntoView({behavior:'smooth',block:'start'})}
function updateStats(){const total=wordsDict().length,shown=wordsFiltered.length;document.getElementById('wordsStats').innerHTML=`Showing <strong>${shown.toLocaleString()}</strong> of <strong>${total.toLocaleString()}</strong> entries`}
